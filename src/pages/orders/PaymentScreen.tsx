...
const PaymentScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        ...
        console.warn(
          "⚠️ [PaymentScreen] Failed to fetch order items for bill:",
          err,
        );
      }
    };
  ...
  console.log(
    `💳 [PaymentScreen] mount — orderId=${orderId}, total=${totalAmount}, customer=${customerName}`,
  );
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      if (successRecord) {
        ...
        console.log(
          `⚠️ [PaymentScreen] Order ${orderId} already paid: amount=${paid}`,
        );
      ...
      console.log(
        `📋 [PaymentScreen] history display count=${filteredList.length}`,
      );
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (successRecord) {
            ...
            console.log(
              `✅ [PaymentScreen] Auto-detected VNPay success! apiAmount=${paidAmount}`,
            );
  ...
  useEffect(() => {
    const sub = AppState.addEventListener("change", async (nextState) => {
      ...
      if (
        ...
        console.log(
          "🔄 [PaymentScreen] AppState active → checking VNPay status...",
        );
        try {
          const res = await getPaymentHistory(orderId);
          const list = (res as any)?.data || (res as any)?.rows || res || [];
          ...
          if (successRecord) {
            const paidAmount = parseFloat(successRecord.amount || totalAmount);
            console.log(
              `✅ [PaymentScreen] AppState: VNPay success detected! amount=${paidAmount}`,
            );
            setConfirmedAmount(paidAmount);
            setOrderAlreadyPaid(true);
          ...
          console.warn("⚠️ [AppState check] error:", e);
        }
      }
    });
    return () => sub.remove();
  ...
  const handleCashPayment = async () => {
    ...
    try {
      setProcessing(true);
      console.log(
        `💵 [PaymentScreen] CASH → orderId=${orderId}, cashReceived=${cashReceived}`,
      );

      const res = await payCash(orderId, cashReceived);
      console.log(
        `✅ [PaymentScreen] payCash full response:`,
        JSON.stringify(res, null, 2),
      );
  ...
  const handleVNPayPayment = async () => {
    try {
      ...
      console.log(
        `🏦 [PaymentScreen] VNPay URL response:`,
        JSON.stringify(res, null, 2),
      );
      ...
      if (!url) {
        console.error(
          "❌ [PaymentScreen] VNPay URL not found in response:",
          res,
        );
        Toast.show({
          type: "error",
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              try {
                ...
                if (successRecord) {
                  console.log(
                    "✅ [PaymentScreen] VNPay confirmed by BE → playing TTS",
                  );
                  ...
                  console.log(
                    "⚠️ [PaymentScreen] VNPay not yet successful on BE",
                  );
                  Toast.show({
                    type: "error",

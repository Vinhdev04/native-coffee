  ...
  async print(data: PrintData): Promise<boolean> {
    try {
      if (Platform.OS !== 'android') {
        Toast.show({ type: 'info', text1: 'Tính năng in chỉ hỗ trợ trên thiết bị Android' });
        return false;
      }
      ...
      */
      ...
      SunmiPrinter.printerText(`Khách: ${data.customerName || 'Khách vãng lai'}\n`);
      SunmiPrinter.printerText('--------------------------------\n');

      // Items Table
      SunmiPrinter.setFontWeight(true);
      SunmiPrinter.printColumnsText(['Tên món', 'SL', 'Thành tiền'], [18, 4, 10], [0, 1, 2]);
      ...
      data.items.forEach((item: any) => {
        ...
        SunmiPrinter.printColumnsText([name, String(qty), lineTotal], [18, 4, 10], [0, 1, 2]);
        
        // In options nếu có
        if (item.selectedAttributes && item.selectedAttributes.length > 0) {
          const optStr = item.selectedAttributes.map((a: any) => a.name).join(', ');
      ...
      SunmiPrinter.setFontWeight(true);
      SunmiPrinter.printerText(`TỔNG CỘNG: ${data.totalPrice.toLocaleString('vi-VN')}đ\n`);
      SunmiPrinter.setFontWeight(false);
      SunmiPrinter.setFontSize(24);

            ...
            <View style={s.receiptBody}>
              ...
              <View style={s.itemsSection}>
                <View style={s.itemHeader}>
                  <Text style={[s.itemHeadLabel, { flex: 2 }]}>Tên món</Text>
                  <Text style={[s.itemHeadLabel, { flex: 0.5, textAlign: 'center' }]}>SL</Text>
                  <Text style={[s.itemHeadLabel, { flex: 1, textAlign: 'right' }]}>T.Tiền</Text>
                </View>
                {order.items.map((item, idx) => (
                  <View key={idx} style={s.itemRow}>
                    ...
                    <Text style={[s.itemQty, { flex: 0.5 }]}>{item.quantity}</Text>
                    <Text style={[s.itemTotal, { flex: 1 }]}>{formatCurrency(item.price * item.quantity)}</Text>
                  </View>
                ))}
              </View>
              ...
              <View style={s.totalSection}>
                <View style={s.totalRow}>
                  ...
                  <Text style={s.totalValue}>{formatCurrency(order.totalPrice)}</Text>
                </View>
                <View style={s.totalRow}>
                ...
                <View style={[s.totalRow, { marginTop: 10 }]}>
                  ...
                  <Text style={s.grandTotalValue}>{formatCurrency(order.totalPrice)}</Text>
                </View>
              </View>
...
const s = StyleSheet.create({
  ...
  topActions: { 
    ...
    padding: 16, backgroundColor: COLORS.white, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' 
  },
  ...
  brandName: { fontFamily: FONTS.bold, fontSize: 20, color: '#111827', letterSpacing: 1 },
  brandAddress: { fontFamily: FONTS.regular, fontSize: 11, color: COLORS.textMuted, textAlign: 'center', marginTop: 4 },
  brandPhone: { fontFamily: FONTS.regular, fontSize: 11, color: COLORS.textMuted, marginTop: 2 },
  
  divider: { height: 1, backgroundColor: '#E5E7EB', marginVertical: 15 },
  dashedDivider: { height: 1, borderStyle: 'dashed', borderWidth: 1, borderColor: '#D1D5DB', marginVertical: 15, borderRadius: 1 },

  infoSection: { marginBottom: 10 },
  receiptTitle: { fontFamily: FONTS.bold, fontSize: 16, color: '#111827', textAlign: 'center', marginBottom: 12 },
  ...
  itemHeader: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#F3F4F6', paddingBottom: 8, marginBottom: 10 },
  ...
  itemAttr: { fontFamily: FONTS.regular, fontSize: 11, color: COLORS.textMuted, marginTop: 2 },
  itemQty: { fontFamily: FONTS.bold, fontSize: 13, color: '#111827', textAlign: 'center' },
  itemTotal: { fontFamily: FONTS.bold, fontSize: 13, color: '#111827', textAlign: 'right' },

  totalSection: { paddingVertical: 5 },
  totalRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 },
  totalLabel: { fontFamily: FONTS.medium, fontSize: 14, color: COLORS.textMuted },
  totalValue: { fontFamily: FONTS.bold, fontSize: 14, color: '#111827' },
  grandTotalLabel: { fontFamily: FONTS.bold, fontSize: 16, color: '#111827' },
  grandTotalValue: { fontFamily: FONTS.bold, fontSize: 20, color: COLORS.primary },

  footerSection: { alignItems: 'center', marginTop: 10 },
  qrBox: { padding: 10, backgroundColor: '#F9FAFB', borderRadius: 12, marginBottom: 12 },
  footerThank: { fontFamily: FONTS.bold, fontSize: 13, color: '#111827' },
  footerPowered: { fontFamily: FONTS.regular, fontSize: 10, color: '#9CA3AF', marginTop: 6 },

  bottomCta: { 
    padding: 16, backgroundColor: COLORS.white, borderTopWidth: 1, borderTopColor: '#E5E7EB',
    alignItems: 'center', paddingBottom: Platform.OS === 'ios' ? 34 : 20
  },
  bottomCtaText: { fontFamily: FONTS.bold, fontSize: 15, color: COLORS.primary },
});
